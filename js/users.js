...
const Users = {
    ...
    init() {
        ...
        userCard.innerHTML =
            ...
            '<span class="card-coins"><span class="hw-icon-placeholder" data-icon="coins">' + (window.HWIcon ? window.HWIcon('coins') : '') + '</span> Coins: <span id="card-coins-val">0</span></span><br>' +
            '<span class="card-score"><span class="hw-icon-placeholder" data-icon="leaderboard">' + (window.HWIcon ? window.HWIcon('leaderboard') : '') + '</span> Best: <span id="card-score-val">0</span></span>' +
    ...
    updateUI() {
        ...
        // Update seasonal Halloween Pass
        if (typeof HalloweenPass !== 'undefined' && typeof HalloweenPass.updatePassDisplay === 'function') {
            HalloweenPass.updatePassDisplay();
        }
    },
    ...
    addCoins(n) {
        ...
        if (earned >= DAILY_LIMIT) {
            if (typeof toast === 'function') {
                toast(`${window.HWIcon ? window.HWIcon("coin", "coin-icon") : "🪙"} Límite diario de 1000 coins alcanzado. ¡Vuelve mañana!`, 'error');
            }
            return;
        }
        ...
        if (typeof HalloweenPass !== 'undefined' && typeof HalloweenPass.recordCoinsEarned === 'function') {
            HalloweenPass.recordCoinsEarned(allowed);
        }

        if (allowed < n && typeof toast === 'function') {
            toast(`${window.HWIcon ? window.HWIcon("coin", "coin-icon") : "🪙"} Límite diario casi alcanzado. Solo +${allowed} coins añadidas.`, 'info');
        }
        this.save();
        this.updateUI();
    },
    ...
    _showPreview() {
        ...
        if (cName) cName.innerHTML = window.HWIcon('skull') + ' ' + this.current;
        ...
        if (bp) {
            ...
            if (this.data.equippedBanner && typeof getBanner === 'function') {
                ...
                if (bn) {
                    ...
                    bp.innerHTML = `${window.HWIcon ? window.HWIcon('scroll') : '🎖'} <span class="${bn.css}" style="font-size:0.9em;">${this.current}</span>`;
                }
            } else {
                bp.innerHTML = `${window.HWIcon ? window.HWIcon('scroll') : '🎖'} Sin banner equipado`;
            }
        }
    },
    _applyBannerToName() {
