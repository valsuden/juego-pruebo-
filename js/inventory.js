...
function renderInventory() {
    ...
    for (let runeId in quantities) {
        ...
        if (rune.imageUrl) {
            ...
            glyph.innerHTML = window.HWIcon ? window.HWIcon('rune') : '🔮';
...
window.showTargetSelectionDialog = function(runeName, runeId, color, onSelect) {
    ...
    window[cbName] = function(data) {
        ...
        targets.forEach(t => {
            ...
            btn.innerHTML = `<span>${t.name}</span> <span style="color:#ffdd00; font-size:0.9em;">${window.HWIcon ? window.HWIcon("trophy") : "🏆"}${t.score} 🔥x${t.streak || 0}</span>`;
            btn.style.cssText = `
