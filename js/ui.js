class UIManager {
    ...
    playSound(soundName) {
        if (window.HWAudio) {
            if (soundName === 'correct') { window.HWAudio.init(); window.HWAudio.playCorrect(); return; }
            if (soundName === 'incorrect') { window.HWAudio.init(); window.HWAudio.playIncorrect(); return; }
            if (soundName === 'victory') { window.HWAudio.init(); window.HWAudio.playVictory(); return; }
        }
    ...
    updateTimer(timeLeft, isStreakActive) {
        ...
        if (!timerCircle.classList.contains('timer-streak') && !isStreakActive) {
            if (timeLeft <= 5) {
                timerCircle.style.backgroundColor = 'var(--hw-red-error, #ff3333)';
            } else if (timeLeft <= 30) {
                timerCircle.style.backgroundColor = 'var(--hw-accent, #ff6600)';
            } else {
    ...
    updateEnergyDisplay(magicEnergy) {
        ...
        for (let i = 0; i < 5; i++) {
            if (i < magicEnergy) {
                energySymbols += (window.HWIcon ? window.HWIcon('energy') : '💎') + ' ';
            } else {
                energySymbols += (window.HWIcon ? window.HWIcon('energy_empty') : '💀') + ' ';
            }
        }
        ...
        if (magicEnergy <= 1) {
            energyDisplay.style.color = 'var(--hw-red-error, #ff0000)';
        } else if (magicEnergy <= 2) {
            energyDisplay.style.color = 'var(--hw-accent, #ff6600)';
        } else {
            energyDisplay.style.color = 'var(--hw-green-toxic, #39ff14)';
        }
    }
